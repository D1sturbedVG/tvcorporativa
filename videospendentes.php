...
<body class="body">
	...
	<table class="table table-responsive tabelalistar">
		...
		<?php
		$sql = "SELECT * from tb_videos WHERE aceite = 0";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				?>
				<tr>
					<td><a href='video.php?id=<?php echo $row['id_video']?>'><?php echo "<span class='label label-primary'>".$row['id_video']."</span>"?></a></td>
					...
					if ($_SESSION['nivel']<=1) {?>
						...
						<td><a href='eliminarvideo.php?id=<?php echo $row['id_video']?>' onClick="return confirm('Eliminar Video?')"><span class="label label-danger"><span class="glyphicon glyphicon-trash"></span></span></a></td>
						<?php
					}?>
				</tr>
				<?php
			...
			echo "<tr><td colspan='9'>Não existem videos pendentes.</td></tr>";
		}
		?>
	</table>
	<?php
